...
class RequestPayloadResolver extends RequestPayloadValueResolver
{
    /**
    ...
    public function __construct(
        ...
        parent::__construct($this->serializer, $this->validator, $this->translator);
    }
            ...
            if ($this->validator) {
                ...
                if (\count($violations)) {
                    ...
                    foreach ($violations as $violation) {
                        ...
                        $propertyPath = $propertyPath ? ltrim($propertyPath, '[') : '';
                        $validationErrors[] = [$propertyPath => $violation->getMessage()];
                    }

                    throw new HttpException($validationFailedCode, json_encode($validationErrors), new ValidationFailedException($payload, $violations));
                }
            } else {
                try {
                    $payload = $this->$payloadMapper($request, $type, $argument);
